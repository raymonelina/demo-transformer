...
def main():
    ...
    print(f"  Sequence 0 diagonal: {['T' if x else 'F' for x in torch.diag(dummy_tgt_padding_mask[0, 0]).tolist()]}")
    print(f"  Sequence 1 diagonal: {['T' if x else 'F' for x in torch.diag(dummy_tgt_padding_mask[1, 0]).tolist()]}")
    ...
    print(f"Model parameters finite: {all(torch.isfinite(p).all() for p in transformer_model.parameters())}")
    print(f"Input ranges - src: [{dummy_src_ids.min()}, {dummy_src_ids.max()}], tgt: [{dummy_tgt_ids.min()}, {dummy_tgt_ids.max()}]")
    ...
    print(f"Model parameters finite after step: {all(torch.isfinite(p).all() for p in transformer_model.parameters())}")
