...
class DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.debug_mode:
            debug_print(target_input, "layer_target_input", "Target input to decoder layer", "DecoderLayer: ")
            debug_print(encoder_output, "layer_encoder_output", "Encoder output to decoder layer", "DecoderLayer: ")
        """
    ...
    def forward(
        ...
        if self.debug_mode:
            ...
            if src_padding_mask is not None:
                debug_print(src_padding_mask, "src_padding_mask", "Source padding mask", "Decoder: ")
            if tgt_padding_mask is not None:
                debug_print(tgt_padding_mask, "tgt_padding_mask", "Target padding mask", "Decoder: ")
        ...
        if self.debug_mode:
            debug_print(target_embeddings, "token_embeddings", "Token embeddings before positional encoding", "Decoder: ")
        ...
        for i, layer in enumerate(self.decoder_layers):
            if self.debug_mode:
                debug_print(x, f"decoder_layer_{i}_input", f"Input to decoder layer {i}", "Decoder: ")
            ...
            if self.debug_mode:
                debug_print(x, f"decoder_layer_{i}_output", f"Output from decoder layer {i}", "Decoder: ")
            
        # Apply final normalization if using pre-norm
        if self.final_norm is not None:
            x = self.final_norm(x)
            if self.debug_mode:
                debug_print(x, "decoder_final_norm", "Output after final layer normalization", "Decoder: ")
